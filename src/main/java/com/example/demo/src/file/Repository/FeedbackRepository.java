...
public interface FeedbackRepository extends JpaRepository<Feedbacks,Long>
{
    ...
    @Query("SELECT NEW com.example.demo.src.file.dto.response.BoardFeedbackResponse(f.id,f.comment,f.createdAt,f.writers.id)  FROM Feedbacks f WHERE f.boards.id = :boardId")
